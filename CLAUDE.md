...
| `spec2026` | `lbm`, `fotonik3d`, `roms`, `cactus`, `omnetpp`, `gcc`, `zstd` | SPEC CPU2026 (a.k.a. SPEC CPU v8 / cpuv8) refrate, clean -O3 gcc build. **Separate suite from `spec` (CPU2017)** — disjoint benchmark set. `build_spec2026` installs *from the ISO* (`SPEC2026_ISO`, default `/proj/instrument-PG0/cpu2026-1.0.1.iso`) via SPEC's `install.sh`; auto-skipped when the ISO is absent, force-skip with `BUILD_SPEC2026=0`. Install dest `SPEC2026_DEST` (default `$HOME/spec2026`); override run path with `SPEC2026_PATH`. `gcc` is multi-invocation (independently-tracked sub-runs). Same direct-exec launch model as `spec` (runcpu builds/stages; harness execs the staged binary). See `docs/spec2026_integration.md`. |
