...
config_micro_phase(){
    phase_regions="${PHASE_REGIONS:-32}"
    phase_region_mb="${PHASE_REGION_MB:-128}"
    phase_stride="${PHASE_STRIDE:-64}"
    phase_iters="${PHASE_ITERS:-5}"
    phase_cycles="${PHASE_CYCLES:-3}"
    # Zipfian distribution parameters (optional)
    # To disable zipfian: set PHASE_ZIPF_REGION_MB=0 or unset these variables
    ...
    phase_zipf_theta="${PHASE_ZIPF_THETA:-0.75}"                # 0.75 for ~1GB hot in 4GB total
    phase_zipf_rate="${PHASE_ZIPF_RATE:-0}"                     # 0 = unlimited
    phase_zipf_workload_size="${PHASE_ZIPF_WORKLOAD_SIZE:-2000000000}"   # total accesses (0 = unlimited, runs until sequential done)
    # Threading parameters
    phase_threads="${PHASE_THREADS:-4}"                         # number of threads for phase access
    zipf_threads="${ZIPF_THREADS:-4}"                           # number of threads for zipfian access

    # Zipfian lifecycle parameters
    phase_zipf_repeats="${PHASE_ZIPF_REPEATS:-0}"              # Number of times to repeat the zipfian workload (after first run)
    phase_zipf_sleep_sec="${PHASE_ZIPF_SLEEP_SEC:-0}"          # Sleep duration between repeats in seconds
    phase_zipf_unmap="${PHASE_ZIPF_UNMAP:-0}"                  # Whether to unmap memory during sleep (0 or 1)
}

build_micro_phase(){
    (cd "$CUR_PATH/microbench/micro_phase" && g++ -O3 -std=c++17 -pthread -Wall -Wextra phase_toggle.cpp -o phase_toggle)
}
